...
function format_mod(int $v): string { return ($v >= 0 ? '+' : '') . (string)$v; }
...
$rollHistory    = load_roll_history($db, $userId, $characterId, 10);
...
<head>
  ...
  <?php if ($readonly): ?>
    <style>
      ...
      a, button, input, textarea, [role="button"], [onclick] { pointer-events:none!important; cursor:default!important; }
  ...
  <section class="glass sheet">
    ...
    <div class="sheet-topbar">
      ...
      <div class="topbar-grid">
        <div class="field">
          <label>Name</label>
          <input type="text" value="<?= h((string)($profile['name'] ?? '')) ?>" readonly />
        </div>
        <div class="field">
          <label>Pronouns</label>
          <input type="text" value="<?= h((string)($profile['pronouns'] ?? '')) ?>" readonly />
        </div>
        <div class="field">
          <label>Heritage</label>
          <input type="text" value="<?= h((string)($profile['heritage_name'] ?? '—')) ?>" readonly />
        </div>
        <div class="field">
          <label>Community</label>
          <input type="text" value="<?= h((string)($profile['community_name'] ?? '—')) ?>" readonly />
        </div>
        <div class="field">
          <label>Class</label>
          <input type="text" value="<?= h((string)($profile['class_name'] ?? '—')) ?>" readonly />
        </div>
        <div class="field">
          <label>Subclass</label>
          <input type="text" value="<?= h((string)($profile['subclass_name'] ?? '—')) ?>" readonly />
        </div>
        <div class="field">
          <label>Level</label>
          <input type="number" value="<?= (int)($profile['level'] ?? 1) ?>" readonly />
        </div>
      </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="sheet-block">
            ...
            <div class="field mt-3">
              ...
              <textarea readonly><?= h($hopeFeatureDesc) ?></textarea>
            </div>
        ...
        <div class="col-12 col-lg-6">
          <div class="sheet-block">
            ...
            <div class="inner-card">
              ...
              <input type="hidden" id="invItemID" value="0" />
              <div class="row g-2 align-items-end">
                <div class="col-12 col-lg-4">
                  <div class="field mb-0"><label>Name</label><input id="invName" type="text" maxlength="120" /></div>
                </div>
                <div class="col-12 col-lg-5">
                  <div class="field mb-0"><label>Description</label><input id="invDesc" type="text" maxlength="500" /></div>
                </div>
                <div class="col-7 col-lg-2">
                  <div class="field mb-0"><label>Amount</label><input id="invAmt" type="number" min="0" max="9999" value="1" /></div>
                </div>
                <div class="col-5 col-lg-1 d-grid">
                  <button class="btn btn-outline-light" id="btnInvSave" type="button">
                    <i class="bi bi-plus-lg me-1"></i><span id="invBtnLabel">Add</span>
                  </button>
                </div>
              </div>
              ...
              <div id="invListWrap">
                <?php if (!$inventoryRows): ?>
                  <div class="inv-empty">No items added yet.</div>
                <?php else: ?>
                    ...
                    <table class="inv-table" id="invTable">
                      ...
                      <tbody>
                        <?php foreach ($inventoryRows as $r): ?>
                          <tr data-itemid="<?= (int)$r['itemID'] ?>"
                              data-item="<?= h((string)$r['Item']) ?>"
                              data-description="<?= h((string)($r['Description'] ?? '')) ?>"
                              data-amount="<?= (int)($r['Amount'] ?? 0) ?>">
                            <td><?= h((string)$r['Item']) ?></td>
                            <td class="muted"><?= h((string)($r['Description'] ?? '')) ?></td>
                            <td style="text-align:right;font-weight:900;"><?= (int)($r['Amount'] ?? 0) ?></td>
